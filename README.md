...
- AST 数据构建：先去重、再分层切分，再从 train split 挖掘动态词表，最后在各 split 内生成对抗变体，避免同源样本泄漏。
...
src/
  ...
  ast_vocabulary.py         # 基于训练集的动态 AST 词表扩展
...
.venv_submit/bin/python scripts/build_ast_dataset.py \
  ...
  --dynamic-vocab-top-k 80
```

构建 strong AST 数据集可直接运行 strong 实验脚本，它会复用 mild manifest 中的数据源。
动态词表默认启用；如需做消融对照，可追加 `--no-dynamic-vocab`，得到“仅外部固定词库”的 AST 数据。
...
当前词库覆盖联系渠道、银行金融、电商促销、房产、教育、博彩、发票办证、运营商服务、退订伪装和匿名化占位符等场景。数字混淆、插入符号、联系方式正则、攻击 profile、混合攻击顺序和上下文提示也集中放在词库的 `runtime_config` 中，避免把业务词和策略表硬塞在 Python 代码里。

在固定词库之外，`src/ast_vocabulary.py` 会从训练集自动挖掘 spam 区分度高的关键词，并生成拼音、首字母、符号插入、繁体和可选 Word2Vec 相似词扰动。生成的 `dynamic_vocab.json` 会与外部固定词库合并使用，并复制到训练输出目录；数据集构建、置信度搜索和 Web UI 都通过同一个 `ChineseSpamTextAttacker` 加载“固定词库 + 动态词表”，保证训练和网页 AST 规则一致。`segment_for_project` 也会把这些 AST 领域词注册进 jieba，避免攻击规则认识领域词、分词器却拆碎领域词。
...
注意：如果修改固定词库或启用/关闭动态词表，必须重新构建 AST 数据集并重新训练/评估，旧的 `output/submission_*` 指标不会自动代表新 AST 版本结果。
...
# 消融：关闭动态词表，只使用外部固定词库
NO_DYNAMIC_VOCAB=1 REBUILD_MILD=1 bash scripts/run_wsl2_full_matrix.sh
```
...
以下结果来自当前已经训练完成的默认矩阵。新增的动态词表、Focal Loss、BiLSTM-Attention 和 `ensemble_vote` 需要按上面的数据构建与 `--full-matrix` 命令重训后再更新指标。
