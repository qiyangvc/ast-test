...
src/ast_vocabulary.py         # 训练集动态 AST 词表扩展
...
动态词表默认启用，并且只从 train split 挖掘关键词。消融实验可追加 `--no-dynamic-vocab`，保留固定外部词库但关闭训练集动态扩展。
...
覆盖域包括联系渠道、银行金融、电商促销、房产、教育、博彩、发票办证、运营商服务、退订伪装、匿名化占位符等。数字混淆、插入符号、联系方式正则、攻击 profile、混合攻击顺序和上下文提示也集中放在词库的 `runtime_config` 中。

本版新增 `src/ast_vocabulary.py`，会从训练集自动挖掘 spam 区分度高的关键词，并生成拼音、首字母、符号插入、繁体和可选 Word2Vec 相似词扰动。生成的 `dynamic_vocab.json` 与外部固定词库合并使用，并复制到训练输出目录。训练集构建、置信度搜索和图形化界面均使用同一个 `ChineseSpamTextAttacker` 加载“固定词库 + 动态词表”，因此不会出现训练和网页 AST 规则不一致。`segment_for_project` 会把 AST 领域词注册进 jieba，Word2Vec、模型输入和网页演示使用同一套分词口径。
...
Precision、Recall 和 F1-Score 均以 spam 类作为正类，同时在 JSON 中保留 macro_f1、weighted_f1、normal_f1 等完整指标。
...
注意：上面的训练指标来自已生成的默认矩阵 `data/ast_experiment*` 和 `output/submission_*` 产物。若修改 `lexicons/chinese_spam_ast_lexicon.json`、启用/关闭动态词表，或采用新增的 Focal Loss、BiLSTM-Attention、`ensemble_vote` 扩展矩阵，必须重新构建 AST 数据集并重新运行训练、clean/AST/UCI/跨扰动/置信度攻击评估后，才能更新这些指标。
