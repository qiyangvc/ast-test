#!/usr/bin/env python
"""测试高级优化器：SWA + Lookahead + Label Smoothing + 最佳集成"""
import os
import sys
import logging
import numpy as np
...
from src.advanced_optimizers import (
    SWALossTrainer, LookaheadTrainer, LabelSmoothingTrainer, BestModelEnsemble
)
...
def test_swa(x_train, y_train, x_val, y_val, x_test, y_test):
    """测试SWA权重平均"""
    logging.info("\n" + "=" * 60)
    logging.info("创新点10: SWA随机权重平均 (swa_start=5)")
    logging.info("=" * 60)
    ...
    trainer = SWALossTrainer(model, swa_start=5)
    history = trainer.fit(
    ...
    logging.info(f"SWA测试准确率: {test_acc:.4f}")
    return test_acc, model


def test_lookahead(x_train, y_train, x_val, y_val, x_test, y_test):
    """测试Lookahead优化器"""
    logging.info("\n" + "=" * 60)
    logging.info("创新点11: Lookahead优化器 (k=5, alpha=0.5)")
    logging.info("=" * 60)
    ...
    trainer = LookaheadTrainer(model, k=5, alpha=0.5)
    history = trainer.fit(
        x_train, y_train, x_val, y_val,
        epochs=15,
        batch_size=16,
    ...
    logging.info(f"Lookahead测试准确率: {test_acc:.4f}")
    return test_acc, model


def test_label_smoothing(x_train, y_train, x_val, y_val, x_test, y_test):
    """测试标签平滑"""
    logging.info("\n" + "=" * 60)
    logging.info("创新点12: Label Smoothing (smoothing=0.1)")
    logging.info("=" * 60)
    ...
    trainer = LabelSmoothingTrainer(model, smoothing=0.1)
    history = trainer.fit(
        x_train, y_train, x_val, y_val,
        epochs=15,
        batch_size=16,
    ...
    logging.info(f"Label Smoothing测试准确率: {test_acc:.4f}")
    return test_acc, model
...
def main():
    ...
    logging.info("=" * 60)
    logging.info("高级优化器测试")
    logging.info("创新点10: SWA随机权重平均")
    logging.info("创新点11: Lookahead优化器")
    logging.info("创新点12: Label Smoothing")
    logging.info("创新点13: 最佳模型集成")
    logging.info("=" * 60)
    ...
    swa_acc, _ = test_swa(x_train, y_train, x_val, y_val, x_test, y_test)
    results['SWA'] = swa_acc

    lookahead_acc, _ = test_lookahead(x_train, y_train, x_val, y_val, x_test, y_test)
    results['Lookahead'] = lookahead_acc

    label_smooth_acc, _ = test_label_smoothing(x_train, y_train, x_val, y_val, x_test, y_test)
    results['LabelSmoothing'] = label_smooth_acc

    best_ensemble_acc, _ = test_best_ensemble(x_train, y_train, x_val, y_val, x_test, y_test)
    results['BestEnsemble'] = best_ensemble_acc
    ...
    logging.info(f"最佳单项模型准确率:   {best_previous:.4f}")
    logging.info("-" * 50)

    for name, acc in sorted(results.items(), key=lambda x: x[1], reverse=True):
        improvement = (acc - baseline_acc) * 100
        best_improvement = (acc - best_previous) * 100
        logging.info(f"{name}: {acc:.4f} (vs基线: {improvement:+.2f}%, vs最佳: {best_improvement:+.2f}%)")

    logging.info("-" * 50)
    best_method = max(results, key=results.get)
    best_acc = results[best_method]
    logging.info(f"本次最佳: {best_method} ({best_acc:.4f})")
